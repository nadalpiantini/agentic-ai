...
export function ModelSelector({ value, onChange }: ModelSelectorProps) {
  return (
    <div className="inline-flex items-center gap-1.5 p-1.5 rounded-2xl bg-surface-2 border border-white/5">
      {models.map(model => (
        <button
          key={model.id}
        ...
        >
          <div className={`w-2 h-2 rounded-full ${model.color} ${value === model.id ? 'opacity-100' : 'opacity-40'}`} />
          {model.label}
        </button>
      ))}
    </div>
